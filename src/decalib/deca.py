...
from .datasets import datasets, detectors
from .utils.config import cfg
torch.backends.cudnn.benchmark = True

class Preprocess():
    def __init__(self, crop_size=224, scale=1.25):
        '''
    ...
    def detect_and_crop(self, image):
        ...
        return dst_image * 255, tform.params, detected_flag
...
class DECA(nn.Module):
    ...
    def _setup_renderer(self, model_cfg):
        self.cfg.rasterizer_type='pytorch3d'
    ...
    def img_to_3dcoeff(self, image):
        cropped_image, tform_params, detected_flag = self.preprocess.detect_and_crop(image)
        tform = torch.tensor(tform_params, dtype=torch.float32)[None, ...]
        tform = torch.inverse(tform).transpose(1,2).cuda()
        cropped_image = torch.tensor(cropped_image, dtype=torch.float32).permute(2, 0, 1).cuda()[None] / 255.0
        codedict = self.encode(cropped_image)
        codedict["tform"] = tform
        codedict["height"] = image.shape[0]
        codedict["width"] = image.shape[1]
        return codedict, detected_flag

    def render_from_3dcoeff(self, codedict):
        h, w = codedict["height"], codedict["width"]
        tform = codedict["tform"]
        ## decode
        # verts, landmarks2d, landmarks3d = self.flame(shape_params=codedict['shape'], expression_params=codedict['exp'], pose_params=None)
        verts, landmarks2d, landmarks3d = self.flame(shape_params=codedict['shape'], expression_params=codedict['exp'], pose_params=codedict['pose'])

        albedo = torch.zeros([1, 3, self.uv_size, self.uv_size], device="cuda") 
        ## projection
        trans_verts = util.batch_orth_proj(verts, codedict['cam']); trans_verts[:,:,1:] = -trans_verts[:,:,1:]
        ...
        shape_images, _, grid, alpha_images = self.render.render_shape(verts, trans_verts, h=h, w=w, images=None, return_grid=True)
        detail_normal_images = F.grid_sample(uv_detail_normals, grid, align_corners=False)*alpha_images
        shape_detail_images = self.render.render_shape(verts, trans_verts, detail_normal_images=detail_normal_images, h=h, w=w, images=None)
        decode_dict = {}
        decode_dict["verts"] = trans_verts
        decode_dict["shape_image"] = shape_images.detach().cpu().numpy()[0, 0] * 255
        decode_dict["shape_detail_image"] = shape_detail_images.detach().cpu().numpy()[0, 0] * 255
        return decode_dict
