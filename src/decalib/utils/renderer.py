...
class SRenderY(nn.Module):
    def __init__(self, image_size, obj_filename, uv_size=256, rasterizer_type='pytorch3d'):
        ...
        # clone faces and face_colors for reuse in video 
        self.face_colors_backup = face_colors.clone()
        self.faces_backup = faces.clone()
    ...
    def add_ambient_light(normals, ambient_light=None):
        '''
            normals: [bz, nv, 3]
        ...
        if ambient_light is None:
            ambient_light = torch.tensor([180, 180, 180], dtype=torch.float32).to(normals.device) / 255.
            ambient_light = ambient_light[None, :]  # 扩展为 [1, 3]

        # Add ambient light
        ambient_shading = ambient_light[:, None, :]#.repeat(1, normals.shape[1], 1)  # expand to [bz, nv, 3]
    ...
    def render_shape(self, vertices, transformed_vertices, colors = None, images=None, detail_normal_images=None, 
                lights=None, return_grid=False, uv_detail_normals=None, h=None, w=None, light_type='point', mask=None):
        '''
        -- rendering shape with detail normal map
        '''
        # for cycle reuse
        self.faces = self.faces_backup.clone().to(vertices.device)
        self.face_colors = self.face_colors_backup.clone().to(vertices.device)
