...
def find_best_frame(kp_source, kp_drivings, cpu=False):
    ...
    kp_source = normalize_kp(kp_source["lmks"])
    norm  = float('inf')
    frame_num = 0
    for i, kp_driving in enumerate(kp_drivings):
        # kp_driving = fa.get_landmarks(255 * image)[0]
        kp_driving = normalize_kp(kp_driving["lmks"])
...
def motion_sync(sequence_driver_det, reference_det, per_landmark_align=True, best_idx=0):
    assert type(sequence_driver_det) is list
    assert type(sequence_driver_det[0]) is type(reference_det) 
    ...
    trans_mats = [] 
    for det in sequence_driver_det: 
        trans_mats.append(det["trans_mat"] @ np.linalg.inv(sequence_driver_det[best_idx]["trans_mat"]))
