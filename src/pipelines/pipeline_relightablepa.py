...
class RelightablepaPipeline(DiffusionPipeline):
    ...
    def _encode_app_image(
        self, 
        image_app: torch.Tensor, 
        do_classifier_free_guidance: bool,
    ):
        # Get latents_light
        app_latents = self.app_embedder(image_app)

        if do_classifier_free_guidance:
            negative_light_latents = torch.zeros_like(app_latents)

            # For classifier free guidance, we need to do two forward passes.
            # Here we concatenate the unconditional and text embeddings into a single batch
            # to avoid doing two forward passes
            app_latents = torch.cat([negative_light_latents, app_latents])
    ...
    def __call__(
        ...
        image_head: Union[torch.FloatTensor],
        image_light: Union[torch.FloatTensor],
        image_mask: Union[torch.FloatTensor],
        image_app: Union[torch.FloatTensor],
        height: int = 576,
        width: int = 1024,
        ...
        head_latents = self._encode_head_image(
            image_head, do_classifier_free_guidance=self.do_classifier_free_guidance,
        )
        light_latents = self._encode_light_image(
            image_light, do_classifier_free_guidance=self.do_classifier_free_guidance,
        )
        app_latents = self._encode_app_image(
            image_app, do_classifier_free_guidance=self.do_classifier_free_guidance,
        )
        ...
        image_latents = image_latents * (1 - image_mask)
        ...
        head_latents = einops.rearrange(head_latents, '(b f) c h w -> b f c h w', f=num_frames)
        light_latents = einops.rearrange(light_latents, '(b f) c h w -> b f c h w', f=num_frames)
        app_latents = einops.rearrange(app_latents, '(b f) c h w -> b f c h w', f=num_frames)
            ...
            for i, t in enumerate(timesteps):
                ...
                for idx in indices:
                    _noise_pred = self.unet(
                        ...
                        head_latents=head_latents[:, idx].flatten(0, 1),
                        light_latents=light_latents[:, idx].flatten(0, 1),
                        app_latents=app_latents[:, idx].flatten(0, 1),
